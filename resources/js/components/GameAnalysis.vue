...
// The modal is visible whenever this component is mounted — focus trap activates
// on mount and deactivates automatically in the unmount hook.
...
async function exportToPdf() {
    ...
    try {
        ...
        if (!target) {
            notify('Neizdevās atrast eksportējamo saturu', 'error');
            return;
        }
        ...
        if (imgHeight <= pageHeight - margin * 2) {
            ...
            const sliceHeightPx = (pageContentHeight * canvas.width) / imgWidth;
            let renderedPx = 0;
            let isFirst = true;
        ...
        const filename = `analize-partija-${props.gameId}-${Date.now()}.pdf`;
        pdf.save(filename);
        notify('PDF eksportēts veiksmīgi!', 'success');
    } catch (err) {
        console.error('PDF export failed:', err);
        notify('Neizdevās eksportēt PDF', 'error');
    } finally {
...
/**
 * Export the visible analysis pane as a single PNG. Uses the same html2canvas
 * snapshot but skips jsPDF entirely — saves bandwidth, no second dependency.
 */
async function exportToPng() {
    ...
    try {
        ...
        const filename = `analize-partija-${props.gameId}-${Date.now()}.png`;
        ...
        notify('PNG eksportēts veiksmīgi!', 'success');
    } catch (err) {
        console.error('PNG export failed:', err);
        notify('Neizdevās eksportēt PNG', 'error');
    } finally {
        isExporting.value = false;
    }
}
...
<template>
    <div class="fixed inset-0 z-50 bg-black/80 backdrop-blur-sm overflow-y-auto p-4" @click.self="emit('close')">
        <div ref="dialogRef"
            ...
            class="max-w-6xl mx-auto my-4 bg-zinc-900 border border-white/10 rounded-3xl shadow-2xl focus:outline-none">
            <!-- Header -->
            <div class="p-5 border-b border-white/5 flex items-center justify-between flex-wrap gap-3">
                ...
                <div class="flex items-center gap-2">
                    <button @click="exportToPdf" :disabled="isExporting"
                        type="button"
                        aria-label="Eksportēt analīzi PDF formātā"
                        title="Eksportēt analīzi PDF formātā"
                        class="px-3 py-2 text-xs font-bold rounded-xl border border-white/10 text-zinc-400 hover:text-amber-400 disabled:opacity-40 disabled:hover:text-zinc-400 transition-all focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-amber-400/60">
                        {{ isExporting ? '…' : '⬇ PDF' }}
                    </button>
                    <button @click="exportToPng" :disabled="isExporting"
                        type="button"
                        aria-label="Eksportēt analīzi PNG attēlā"
                        title="Eksportēt analīzi PNG attēlā"
                        class="px-3 py-2 text-xs font-bold rounded-xl border border-white/10 text-zinc-400 hover:text-amber-400 disabled:opacity-40 disabled:hover:text-zinc-400 transition-all focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-amber-400/60">
                        {{ isExporting ? '…' : '🖼 PNG' }}
                    </button>
                    <button @click="handleShare"
                        type="button"
                        :aria-label="shareCopied ? 'Saite nokopēta' : 'Izveidot kopīgošanas saiti'"
                        :title="shareCopied ? 'Nokopēts!' : 'Izveidot kopīgošanas saiti'"
                        :class="['px-3 py-2 text-xs font-bold rounded-xl border transition-all focus-visible:outline-none focus-visible:ring-2',
                            shareCopied
                                ? 'border-emerald-500/30 text-emerald-400 bg-emerald-500/10 animate-pop-success focus-visible:ring-emerald-400/60'
                                : 'border-white/10 text-zinc-400 hover:text-amber-400 focus-visible:ring-amber-400/60']">
                        {{ shareCopied ? '✓' : '🔗' }}
                    </button>
                    <button @click="emit('close')"
                        type="button"
                        aria-label="Aizvērt analīzes logu"
                        title="Aizvērt"
                        class="px-3 py-2 text-xs font-bold rounded-xl border border-white/10 text-zinc-400 hover:text-white transition-all focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-white/40">✕</button>
                </div>
            </div>

...
const sortOptions = [
    { label: 'Jaunākās', value: '-created_at' },
    { label: 'Vecākās', value: 'created_at' },
    { label: 'Gājieni ↓', value: '-total_moves' },
    { label: 'Gājieni ↑', value: 'total_moves' },
];
...
const debouncedSearch = useDebounce(() => fetchGames(1), 400);

const handleGameCreated = () => {
    isUploadOpen.value = false;
    fetchGames(1);
    notify('Partija augšupielādēta veiksmīgi!', 'success');
};
    ...
    <div class="min-h-screen p-6 lg:p-10 text-white">
        <div class="max-w-7xl mx-auto">
            ...
            <div class="flex flex-col lg:flex-row justify-between items-start lg:items-center mb-10 gap-6">
                <div>
                    <h1 class="text-4xl font-black tracking-tight"><span class="text-amber-400">♟</span> Partijas</h1>
                    <p class="text-zinc-500 text-sm mt-1">{{ gamesStore.pagination.total }} partija(s) sistēmā</p>
                </div>

                <div class="flex items-center gap-3 flex-wrap">
                    <select v-model="currentFilter"
                        class="bg-zinc-900 border border-white/10 rounded-xl px-4 py-2.5 text-sm text-zinc-400 focus:outline-none focus:border-amber-500/50">
                        <option value="">Visi rezultāti</option>
                    ...
                    <button @click="isUploadOpen = true"
                        class="bg-gradient-to-r from-amber-500 to-amber-600 text-black px-6 py-2.5 rounded-xl font-bold shadow-lg shadow-amber-500/20 hover:from-amber-400 hover:to-amber-500 transition-all flex items-center gap-2">
                        ⬆ Augšupielādēt PGN
                    </button>
                </div>
            </div>
            ...
            <div v-else-if="gamesStore.games.length > 0">
                <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-5 mb-10">
                    <GameCard v-for="game in gamesStore.games" :key="game.id" :game="game"
                        @analyze="handleAnalyze" @delete="handleDelete" />
                </div>
